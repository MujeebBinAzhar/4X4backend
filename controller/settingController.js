...
const addStockManagementSetting = async (req, res) => {
  try {
    const setting = req.body;
    const stockManagementSetting = new Setting(setting);
    await stockManagementSetting.save();
    res.status(201).send({
      message: "Stock management setting added successfully!",
    });
  } catch (err) {
    res.status(500).send({
      message: err.message,
...
const updateStockManagementSetting = async (req, res) => {
  try {
    ...
    const stockManagementSetting = await Setting.findOneAndUpdate(
      ...
      {
        $set: {
          "setting.low_stock_threshold": setting.low_stock_threshold,
    ...
    res.status(200).send({
      message: "Stock management setting updated successfully!",
      data: stockManagementSetting,
    });
  } catch (err) {
    res.status(500).send({
...
module.exports = {
  ...
  addStockManagementSetting,
  getStockManagementSetting,
  updateStockManagementSetting,
};
