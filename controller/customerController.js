...
const addShippingAddress = async (req, res) => {
  try {
    ...
    // Find the customer by ID and update the shippingAddresses field
    const result = await Customer.updateOne(
      ...
      { upsert: true } // Only update if the customer exists
    );

    if (result.nModified > 0 || result.upserted) {
      // Fetch the customer with the latest shipping address
      ...
      const latestShippingAddress =
        updatedCustomer.shippingAddresses[updatedCustomer.shippingAddresses.length - 1];

      return res.send(latestShippingAddress);
    } else {
...
const getDefaultShippingAddress = async (req, res) => {
  try {
    ...
    const customer = await Customer.findById(customerId, { shippingAddresses: 1 });
    ...
    const defaultAddress = customer.shippingAddresses.find((addr) => addr.default);

    if (!defaultAddress) {
      return res.status(404).send({ message: "Default shipping address not found!" });
    }

    return res.send(defaultAddress);
  } catch (err) {
    res.status(500).send({
      message: err.message,
    });
...
const getShippingAddress = async (req, res) => {
  try {
    ...
    res.send({ shippingAddress: customer?.shippingAddresses || [] });
